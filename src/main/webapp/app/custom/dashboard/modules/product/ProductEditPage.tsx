...
const mapEntityToState = (entity: IProduitDemande): ProductFormState => ({
  typeProduit: entity.typeProduit ?? '',
  nomProduit: entity.nomProduit ?? '',
  description: entity.description ?? '',
  quantite: entity.quantite !== undefined && entity.quantite !== null ? String(entity.quantite) : '',
  unite: entity.unite ?? '',
  prix: entity.prix !== undefined && entity.prix !== null ? String(entity.prix) : '',
  poidsKg: entity.poidsKg !== undefined && entity.poidsKg !== null ? String(entity.poidsKg) : '',
  volumeTotalCbm: entity.volumeTotalCbm !== undefined && entity.volumeTotalCbm !== null ? String(entity.volumeTotalCbm) : '',
  dimensions: entity.dimensions ?? '',
  hsCode: entity.hsCode ?? '',
  prixCible: entity.prixCible !== undefined && entity.prixCible !== null ? String(entity.prixCible) : '',
  fraisExpedition: entity.fraisExpedition !== undefined && entity.fraisExpedition !== null ? String(entity.fraisExpedition) : '',
  origine: entity.origine ?? '',
  fournisseur: entity.fournisseur ?? '',
  adresseChargement: entity.adresseChargement ?? '',
  adresseDechargement: entity.adresseDechargement ?? '',
  ficheTechniqueUrl: entity.ficheTechniqueUrl ?? '',
  photosUrl: entity.photosUrl ?? '',
  piecesJointesUrl: entity.piecesJointesUrl ?? '',
});

const mapStateToPayload = (values: ProductFormState, id: number): IProduitDemande => {
...
const ProductEditPage = () => {
  ...
  const [formValues, setFormValues] = useState<ProductFormState | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (data) {
          setFormValues(mapEntityToState(data));
        } else {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = mapStateToPayload(formValues, Number(id));
      await axios.put<IProduitDemande>(`api/produit-demandes/${id}`, payload);
      navigate('/dashboard/products', {
        replace: true,
        state: { successMessage: translate('crmApp.produitDemande.updated') },
      });
    } catch (err) {
      const message =
    ...
    <div className="product-create-page py-4">
      <Breadcrumb listClassName="bg-transparent px-0">
        ...
        <BreadcrumbItem active>
          <Translate contentKey="crmApp.produitDemande.home.createOrEditLabel" />
        </BreadcrumbItem>
      </Breadcrumb>
          ...
          <CardBody>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
              ...
              <div className="d-flex gap-2">
                <Button color="secondary" outline tag={Link} to="/dashboard/products" disabled={submitting}>
              ...
              <Row className="g-4">
                ...
                <Col md="4">
                  <FormGroup>
                    ...
                    <Input
                      id="product-ficheTechniqueUrl"
                      value={formValues.ficheTechniqueUrl}
                      onChange={handleChange('ficheTechniqueUrl')}
                    />
                  </FormGroup>
                </Col>
